...
const statusCls = (status: string) => {
  switch (status) {
    ...
    case 'NOT_ON_SALE':
      return 'text-white/30 border border-white/10'
    case 'SOLD_OUT':
      return 'text-orange-400 border border-orange-400/30 bg-orange-400/5'
    default:
      return 'text-white/20 border border-white/10'
  }
}
    ...
    >
      ...
      <div className="flex flex-col gap-2 min-w-0">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className={`font-changa text-[9px] tracking-widest uppercase px-2 py-0.5 ${statusCls(item.status)}`}>
            {STATUS_LABEL[item.status] ?? item.status.replace('_', ' ')}
          </span>
          {!item.isVisible && (
            <span className="font-changa text-[9px] tracking-widest uppercase px-2 py-0.5 text-white/20 border border-white/10">
              Not Yet Available
            </span>
          )}
        </div>
      ...
      <div className="flex items-center gap-3 shrink-0">
        {item.publicUrl && item.publicUrl !== 'TODO' && item.isVisible && (
          <a
            href={item.publicUrl}
            target="_blank"
        ...
        {!item.isVisible && (
          <span className="font-changa text-[10px] uppercase tracking-[0.25em] text-white/20">Available Soon</span>
        )}
      </div>
    </motion.div>
  )
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="max-w-5xl mx-auto px-4 760:px-6 py-16 760:py-20 flex flex-col gap-16">
        ...
        <div className="border-t border-white/10 pt-12 flex flex-col 760:flex-row 760:items-center justify-between gap-6">
          <div className="flex flex-col gap-1">
            ...
            <p className="font-lato text-white/60 text-base max-w-md">
          ...
          <div className="flex flex-col 480:flex-row items-start 480:items-center gap-3 shrink-0">
            <a
              ...
              className="inline-flex items-center gap-2 font-changa text-[10px] uppercase tracking-widest px-5 py-2.5 border border-white/20 text-white/50 hover:text-white hover:border-white/40 transition-colors"
            >
              Email Us
            </a>
            <a
      ...
      <footer className="border-t border-white/10 px-6 760:px-12 990:px-16 py-5">
        <div className="max-w-5xl mx-auto px-4 760:px-6 flex flex-col 480:flex-row items-start 480:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="w-4 h-px bg-blaze shrink-0" aria-hidden="true" />
            <span className="font-changa text-[10px] uppercase tracking-[0.25em] text-white/25">
          ...
          <nav className="flex items-center gap-5">
            <Link
              href="/"
              className="font-changa text-[10px] uppercase tracking-widest text-white/70 hover:text-white transition-colors"
            >
              Home
            </Link>
            <Link
