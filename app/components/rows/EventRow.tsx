  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <span className="text-[11px] font-mono text-text-dark truncate block">{event.title}</span>
        {event.description && (
      ...
      <span className="text-[10px] font-mono text-muted-dark/60 truncate">
        {event.location || <span className="text-muted-dark/30">—</span>}
      </span>

      {/* Date */}
      <div className="flex flex-col">
        <span className="text-[9px] font-mono text-text-dark/80 tabular-nums">
      ...
      <div className="flex justify-end">
        <span
          className={`text-[7px] font-mono uppercase tracking-widest px-1.5 py-0.5 border ${STATUS_COLORS[event.status] ?? 'text-muted-dark border-border-dark'}`}
        >
