...
export function SponsorRow({
  ...
  return (
    ...
    >
      <span className="text-[8px] font-mono text-muted-dark/40 tabular-nums">{String(index + 1).padStart(2, '0')}</span>
      ...
      <div className="w-8 h-8 border border-border-dark flex items-center justify-center bg-white/5 shrink-0 overflow-hidden">
        {sponsor.filePath ? (
          ...
          <span className="text-[7px] font-mono text-muted-dark/50 uppercase tracking-widest leading-none text-center px-0.5">
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-[11px] font-mono text-text-dark truncate">{sponsor.name}</span>
          {sponsor.externalLink && (
            <a
              href={sponsor.externalLink}
              target="_blank"
              rel="noopener noreferrer"
              className="text-muted-dark/40 hover:text-primary-dark transition-colors shrink-0"
      ...
      <div>
        <span
          className={`text-[7px] font-mono uppercase tracking-widest px-1.5 py-0.5 border ${levelColor(sponsor.level)}`}
        >
          {formatLevel(sponsor.level)}
        </span>
      </div>

      <span className="text-[10px] font-mono tabular-nums text-text-dark/70 text-left">
      ...
      <div className="flex items-center gap-1.5">
        ...
        />
        <span
          className={`text-[8px] font-mono uppercase tracking-widest ${sponsor.isActive ? 'text-emerald-400' : 'text-muted-dark/40'}`}
        >
