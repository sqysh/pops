  ...
  return (
    ...
    >
      ...
      <div className="flex-1 px-6 pt-6 pb-4 flex flex-col gap-4">
        {/* Label */}
        <span className={`text-[8px] font-mono uppercase tracking-[0.3em] ${fund.label_color}`}>{fund.label}</span>
        ...
        {/* Description */}
        <p className="font-lato text-text-dark/60 text-sm leading-relaxed">{fund.description}</p>
            ...
            {fund.tiers.map(({ label, amount, note }) => (
              ...
              >
                <div className="flex flex-col">
                  <span className="text-[11px] font-mono text-text-dark/80">{label}</span>
                  {note && <span className="text-[8px] font-mono text-muted-dark/40 italic">{note}</span>}
                </div>
        ...
        {fund.adOptions && (
          <div className="flex flex-col border-t border-border-dark pt-4 mt-1 gap-0">
            {fund.adOptions.map(({ size, note, price }) => (
              ...
              >
                <div className="flex flex-col">
                  <span className="text-[11px] font-mono text-text-dark/80">{size}</span>
                  {note && <span className="text-[8px] font-mono text-muted-dark/40">{note}</span>}
                </div>
                <span className={`text-base font-changa font-black tabular-nums shrink-0 ${fund.label_color}`}>
                  {price}
                </span>
          ...
          <div className="flex flex-col border-t border-border-dark pt-4 mt-1 gap-2">
            {fund.extras.map((item) => (
              <div key={item} className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-mono text-text-dark/60">{item}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {/* Note */}
        {fund.note && (
          <p className="text-[8px] font-mono text-muted-dark/40 leading-relaxed border-t border-border-dark pt-3 mt-1">
            {fund.note}
          </p>
        )}
      </div>
      ...
      <div className="px-6 pb-6 pt-2 flex flex-col gap-2 mt-auto">
        <a
          ...
          className={`inline-flex items-center justify-center gap-2 w-full px-4 py-3 text-[9px] font-mono uppercase tracking-widest transition-colors duration-200 focus-visible:outline-none ${fund.btn}`}
        >
          {fund.cta}
          <ExternalLink className="w-3 h-3 opacity-70" />
        </a>
        {!fund.href.startsWith('mailto') && (
          <p className="text-[7px] font-mono text-muted-dark/25 uppercase tracking-widest text-center">
            Secure · via CueBox
          </p>
        )}
      </div>
